...
def index(request):
    ...
    most_fresh_posts = Post.objects.prefetch_related('author').annotate(Count('comments')).order_by(
        '-published_at')[:5]
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.prefetch_related('author').annotate(Count('comments'))[:20]
